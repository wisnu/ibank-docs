...
```sql
CREATE TABLE ibankcore.rekening_aktivitas_nonfin (
...
CREATE INDEX idx_ran_norek_tgl
    ON ibankcore.rekening_aktivitas_nonfin
...
### ⑤ CREATE TABLE report.rekening_tidak_aktif
...
EOD berjalan (urutan wajib):
  ...
  Step 1 — [BARU] Update tanggal aktivitas terakhir
           ...
           (gabungan: transaksi nasabah + aktivitas non-finansial dari `rekening_aktivitas_nonfin`)
...
> - Membaca tabel `rekening_aktivitas_nonfin` (tabel baru)
...
USING (
    ...
    aktivitas_nonfin AS (
        ...
        FROM ibankcore.rekening_aktivitas_nonfin
...
| `TUTUP_NOL_HARI` | 10 | `REKENING_DORMANT` | Hari default tutup otomatis | `730` | Threshold hari sejak dormant → TUTUP OTOMATIS |
...
- **Giro Korporat** — threshold dormant lebih panjang → `is_custom_dormant = 'T'`, set `jumlah_hari_jadi_dormant = 730`
...
nDefaultHariTutup     = oParamHariTutup.GetInt()              # → 730
