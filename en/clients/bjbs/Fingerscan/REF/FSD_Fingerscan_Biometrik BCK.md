...
- [2. Deskripsi Umum Sistem](#2-deskripsi-umum-sistem)
  - [2.1 Perspektif Sistem](#21-perspektif-sistem)
  - [2.2 Arsitektur High-Level](#22-arsitektur-high-level)
...
| 4 | Fallback mechanism | Mekanisme fallback ke password |
...
│                    FINGERSCAN SERVICE                            │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐          │
│  │ Verification │  │  Enrollment  │  │    Admin     │          │
│  │    Engine    │  │   Service    │  │   Service    │          │
│  └──────────────┘  └──────────────┘  └──────────────┘          │
│                           │                                      │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐          │
│  │    Redis     │  │   Database   │  │     OIDC     │          │
│  │   (Cache)    │  │ (PostgreSQL) │  │  (Identity)  │          │
│  └──────────────┘  └──────────────┘  └──────────────┘          │
└───────────────────────────┬─────────────────────────────────────┘
                            │ WebSocket (Secure)
                            ▼
...
graph LR
    subgraph DataCenter["☁️ DATA CENTER"]
        ...
        subgraph Service["Fingerscan Service"]
            subgraph Modules["Service Modules"]
                VE["Verification<br/>Engine"]
        ...
        VE -.->|"[3] Cache"| Redis
        VE -.->|"[4] Store/Retrieve"| DB
        ES -.->|"[4] Store/Retrieve"| DB
        AS -.->|"[4] Store/Retrieve"| DB
        VE -.->|"[5] Authenticate"| OIDC
    end
    ...
    subgraph Workstation["🖥️ WORKSTATION / CABANG"]
        ...
        subgraph Apps["Business Applications"]
            ...
            CMS["CMS<br/>(Card Management)"]
            Others["Surrounding Apps<br/>(VA, QRIS, etc)"]
        end
        
        subgraph Station["Fingerstation<br/>(Client App)"]
            CM["Capture<br/>Module"]
            DM["Device<br/>Manager"]
            Device["DigitalPersona 4500<br/>(Reader)"]
        end
        
        %% Local connection between apps and station
        DAF -.->|"[1a] Local Call"| CM
        BDS -.->|"[1a] Local Call"| CM
        CMS -.->|"[1a] Local Call"| CM
        Others -.->|"[1a] Local Call"| CM
    ...
    Apps -->|"[1b] REST API<br/>(HTTPS via Network)"| VE
    CM -->|"[2] WebSocket<br/>(Secure/mTLS via Network)"| VE
    CM -->|"[2] WebSocket<br/>(Secure/mTLS via Network)"| ES
    ...
    class DAF,BDS,CMS,Others appStyle
    class VE,ES,AS serviceStyle
    class Redis,DB,OIDC storageStyle
    class CM,DM stationStyle
...
|----|-------|--------|----------|-----------|
| **[1a]** | Apps → Station | **Local** (Workstation) | IPC/Local API | Business apps memanggil Fingerstation secara lokal untuk memicu capture |
| **[1b]** | Apps → Service | **Network** (WAN/VPN) | REST API (HTTPS) | Business apps mengirim request verifikasi ke Fingerscan Service di data center |
| **[2]** | Station → Service | **Network** (WAN/VPN) | WebSocket (mTLS) | Fingerstation mengirim template hasil capture ke service via koneksi aman |
| **[3]** | Service → Cache | **Local** (Data Center) | Redis Protocol | Verification engine mengecek cache untuk template yang sering digunakan |
| **[4]** | Service → Database | **Local** (Data Center) | JDBC/SQL | Menyimpan dan mengambil data template, audit log, dan konfigurasi |
| **[5]** | Service → OIDC | **Local** (Data Center) | OAuth2/OIDC | Autentikasi dan autorisasi user |
| **[6]** | Capture → Device Mgr | **Local** (Workstation) | Internal API | Modul capture mengirim perintah ke device manager |
| **[7]** | Manager → Device | **Local** (Workstation) | USB/SDK | Device manager berkomunikasi dengan hardware fingerprint reader |
...
### 2.1.1 C4 Dynamic Diagram - Verification Flow

Diagram berikut menunjukkan alur dinamis proses verifikasi sidik jari dari awal hingga akhir:
...
**Penjelasan Alur Dinamis:**

| Step | Komponen | Deskripsi | Protokol |
|------|----------|-----------|----------|
| **1** | Approver → App | User menekan tombol approval di aplikasi bisnis | UI Action |
| **2** | App → Session Mgr | Aplikasi mengirim request inisialisasi verifikasi | REST API (HTTPS) |
| **3** | Session Mgr → OIDC | Validasi user aktif dan check hak akses approval | OAuth2/OIDC |
| **4** | Session Mgr → Cache | Membuat session verifikasi dengan TTL 30 detik | Redis SET |
| **5** | Session Mgr → Station | Mengirim perintah capture ke Fingerstation user | WebSocket (mTLS) |
| **6** | Station → Approver | Menampilkan dialog "Letakkan jari Anda" | Desktop UI |
| **7** | Approver → Station | User meletakkan jari di device DigitalPersona | Physical Action |
| **8** | Station → Verif Engine | Mengirim template hasil capture ke service | WebSocket (mTLS) |
| **9** | Verif Engine → Cache | Mengambil template enrolled user dari cache | Redis GET |
| **10** | Verif Engine → DB | Jika cache miss, ambil dari database | JDBC/SQL |
| **11** | Verif Engine | Proses matching 1:1 antara template capture vs enrolled | Internal |
| **12** | Verif Engine → DB | Menyimpan hasil verifikasi dan audit log | JDBC/SQL |
| **13** | Verif Engine → App | Mengembalikan hasil verifikasi (SUCCESS/FAILED) | Callback/Response |
| **14** | App → Approver | Menampilkan hasil approval atau error message | UI |

**Timeline Estimasi:**
- Step 1-5: ~200ms (network + initiation)
- Step 6-7: ~2-5 detik (user action)
- Step 8-11: ~300ms (capture + matching)
- Step 12-14: ~100ms (logging + response)
- **Total: ~3-6 detik** (user-dependent)

---

**C4 Container Diagram Version:**
...
└────────────────────────────────────────────────────────────────────┘
        ...
        ▼                       ▼                       ▼
┌───────────────┐      ┌───────────────┐      ┌───────────────┐
│   Cabang A    │      │   Cabang B    │      │   Cabang N    │
│ ┌───────────┐ │      │ ┌───────────┐ │      │ ┌───────────┐ │
│ │Fingerstat.│ │      │ │Fingerstat.│ │      │ │Fingerstat.│ │
│ └─────┬─────┘ │      │ └─────┬─────┘ │      │ └─────┬─────┘ │
│       │       │      │       │       │      │       │       │
│ ┌─────┴─────┐ │      │ ┌─────┴─────┐ │      │ ┌─────┴─────┐ │
...
|-------|-----------|-----------|
| **Teller/CS** | Melakukan input transaksi | Input transaksi, view status |
| **Approver** | Melakukan otorisasi dengan fingerprint | Approval transaksi |
| **Branch Admin** | Mengelola enrollment di cabang | Enrollment alternate, view station status |
| **IT Administrator** | Mengelola sistem secara keseluruhan | Full access management portal |
| **Auditor** | Mengakses log dan laporan | Read-only audit logs |
| **System** | Automated processes | Scheduler, notifications |
...
```
┌─────────┐      ┌─────────────┐      ┌─────────────┐      ┌──────────┐
│  Admin  │      │  Mgmt Portal│      │   Service   │      │ Station  │
└────┬────┘      └──────┬──────┘      └──────┬──────┘      └────┬─────┘
     │                  │                    │                  │
     │ 1. Select User   │                    │                  │
     │─────────────────►│                    │                  │
...
|---------|------|----------|
| BR-E01 | Minimal 2 jari dari kedua tangan | System enforced |
...
```
┌─────────┐  ┌─────────┐  ┌─────────────┐  ┌─────────────┐  ┌──────────┐
│Business │  │Approval │  │  Fingerscan │  │   Redis     │  │ Station  │
│   App   │  │ Dialog  │  │   Service   │  │   Cache     │  │          │
└────┬────┘  └────┬────┘  └──────┬──────┘  └──────┬──────┘  └────┬─────┘
     │            │              │               │               │
     │ 1. Request │              │               │               │
     │   Approval │              │               │               │
...
Station ─────► Service (every 30s)
   ...
   │
   ▼
Service updates last_heartbeat
   │
   │  If no heartbeat > 60s
   │
   ▼
...
**User Status:**
```
ACTIVE ──[3x fail]──► LOCKED ──[admin unlock]──► ACTIVE
   │                                                │
   └──[HRMIS inactive]──► INACTIVE ◄───────────────┘
```
...
**Station Status:**
```
OFFLINE ──[connect]──► ONLINE ──[disconnect/no heartbeat]──► OFFLINE
    │                     │
    │                     └──[device error]──► ERROR
    │                                            │
